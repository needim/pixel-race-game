...
// lib/pixel-race.js
ig.baked=true;ig.module('pixel-race').requires('impact.game','impact.entity','impact.collision-map','impact.background-map','impact.font').defines(function(){window.FullsizeBackdrop=ig.Image.extend({resize:function(){},draw:function(){if(!this.loaded){return;}
ig.system.context.drawImage(this.data,0,0);}});window.EntityWall=ig.Entity.extend({name:'wall',size:{x:8,y:5},animSheet:new ig.AnimationSheet('media/wall.png',8,5),type:ig.Entity.TYPE.B,speed:1,init:function(x,y,settings){this.addAnim('idle',0.1,[0,1,2,3,4,5]);this.parent(x,y,settings);this.speed=Math.random()>0.50?.1:.2;},update:function(){this.parent();if(this.pos.y>ig.system.height+20){this.kill();}}});window.EntityCoin=ig.Entity.extend({name:'coin',size:{x:4,y:4},animSheet:new ig.AnimationSheet('media/coin.png',4,4),type:ig.Entity.TYPE.B,sound:new ig.Sound('media/coin.ogg'),init:function(x,y,settings){this.addAnim('idle',0.1,[0,1]);this.parent(x,y,settings);},update:function(){this.parent();if(this.pos.y>ig.system.height+20){this.kill();}}});window.EntityPlayer=ig.Entity.extend({name:'player',size:{x:4,y:4},checkAgainst:ig.Entity.TYPE.B,animSheet:new ig.AnimationSheet('media/player.png',4,4),maxVel:{x:50,y:300},friction:{x:300,y:0},speed:4000,bounciness:0.5,sound:new ig.Sound('media/bounce.ogg'),init:function(x,y,settings){this.addAnim('idle',0.01,[0,1]);this.parent(x,y,settings);},update:function(){if(!ig.game.isPaused){if(ig.input.state('left')&&this.pos.x>0){this.pos.x-=1;}else if(ig.input.state('right')&&this.pos.x+4<ig.system.width){this.pos.x+=1;}}
this.parent();},handleMovementTrace:function(res){this.parent(res);},check:function(other){var soundState;if(other.name=='coin'){soundState=store.get('soundState')||'on';if(soundState=='on')
other.sound.play();other.kill();ig.game.score+=1;}else{ig.game.gameOver=true;soundState=store.get('soundState')||'on';if(soundState=='on')
ig.game.gameOverSound.play();}}});window.PixelRaceLoader=ig.Loader.extend({end:function(){for(i in ig.Image.cache){var img=ig.Image.cache[i];if(!(img instanceof FullsizeBackdrop)){this.pixify(img,ig.system.scale);}}
this.parent();},pixify:function(img,s){var ctx=img.data.getContext('2d');var px=ctx.getImageData(0,0,img.data.width,img.data.height);for(var y=0;y<img.data.height;y++){for(var x=0;x<img.data.width;x++){var index=(y*img.data.width+x)*4;var alpha=(x%s==0||y%s==0)?0:0.9;px.data[index+3]=px.data[index+3]*alpha;}}
ctx.putImageData(px,0,0);}});window.PixelRaceGame=ig.Game.extend({clearColor:null,gravity:0,player:null,isSubmitted:false,isPaused:false,score:0,speed:30,backdrop:new FullsizeBackdrop('media/backdrop.png'),font:new ig.Font('media/04b03.font.png'),gameOverSound:new ig.Sound('media/gameover.ogg'),loopSound:new ig.Sound('media/loop.ogg'),init:function(){ig.input.bind(ig.KEY.LEFT_ARROW,'left');ig.input.bind(ig.KEY.RIGHT_ARROW,'right');ig.input.bind(ig.KEY.ENTER,'ok');ig.input.bind(ig.KEY.P,'pause');ig.music.add('media/loop.ogg');var musicState=store.get('musicState')||'on';if(musicState=='on')
ig.music.play();this.player=this.spawnEntity(EntityPlayer,ig.system.width/2-2,ig.system.height-10);},getRow:function(){var row=[];for(var x=0;x<8;x++){row[x]=Math.random()>0.98?1:0;}
return row;},update:function(){if(ig.input.pressed('ok')){this.isSubmitted=false;this.gameOver=false;ig.system.setGame(PixelRaceGame);}
if(ig.input.pressed('pause')){this.isPaused=!this.isPaused;}
if(this.gameOver){var that=this;if(!that.isSubmitted){that.isSubmitted=true;var name=store.get('username');if(typeof name=='undefined')
name='guest';if(location.host!='dev.pixelrace.com'){var leaderboard=new Clay.Leaderboard({id:2618});leaderboard.post({score:parseInt(that.score.floor().toString()),name:name},function(response){});}else{console.log('DEV MODE: ON');}}
return;}
if(!this.isPaused){if(this.speed<200)
this.speed+=(ig.system.tick*(150/this.speed))+0.05;this.screen.y-=ig.system.tick*this.speed;this.player.pos.y-=ig.system.tick*this.speed;var walls=ig.game.getEntitiesByType(EntityWall);var coins=ig.game.getEntitiesByType(EntityCoin);if(this.screen.y<-1){var row=this.getRow();this.screen.y+=8;this.player.pos.y+=8;for(var k=0;k<=8;k++){if(row[k]){var x,y;if(Math.random()>0.80){y=-28;x=k*8;this.spawnEntity(EntityCoin,x,y);}else{y=-28;x=k*8;this.spawnEntity(EntityWall,x,y);}}}
for(var i=0;i<walls.length;i++){walls[i].pos.y-=(ig.system.tick*this.speed-8.5);}
for(var i=0;i<coins.length;i++){coins[i].pos.y-=ig.system.tick*this.speed-8.5;}}}
this.parent();},draw:function(){this.backdrop.draw();var musicState=store.get('musicState')||'on';if(musicState=='off')
ig.music.stop();else if(!this.gameOver&&!this.isPaused)
ig.music.play();if(this.gameOver){this.font.draw('PIXEL RACE',ig.system.width/2,22,ig.Font.ALIGN.CENTER);this.font.draw('Press Enter',ig.system.width/2,38,ig.Font.ALIGN.CENTER);this.font.draw('to Start',ig.system.width/2,46,ig.Font.ALIGN.CENTER);ig.music.stop();}else{this.parent();}
if(this.isPaused){this.font.draw('PAUSED',ig.system.width/2,22,ig.Font.ALIGN.CENTER);ig.music.stop();}
this.font.draw(this.score.floor().toString(),ig.system.width-2,ig.system.height-6,ig.Font.ALIGN.RIGHT);}});window.PixelRaceStarter=ig.Game.extend({clearColor:null,gravity:0,player:null,isSubmitted:false,map:[],score:0,speed:30,backdrop:new FullsizeBackdrop('media/backdrop.png'),font:new ig.Font('media/04b03.font.png'),init:function(){ig.input.bind(ig.KEY.ENTER,'ok');},update:function(){if(ig.input.pressed('ok')){ig.system.setGame(PixelRaceGame);}
return;},draw:function(){this.backdrop.draw();this.font.draw('PIXEL RACE',ig.system.width/2,22,ig.Font.ALIGN.CENTER);this.font.draw('Press Enter',ig.system.width/2,38,ig.Font.ALIGN.CENTER);this.font.draw('to Start',ig.system.width/2,46,ig.Font.ALIGN.CENTER);this.font.draw('coins +1',ig.system.width/2,66,ig.Font.ALIGN.CENTER);this.parent();}});ig.main('#canvas',PixelRaceStarter,30,64,96,5,PixelRaceLoader);});
...
other.sound.play();other.kill();ig.game.score+=150;}else{ig.game.gameOver=true;soundState=store.get('soundState')||'on';if(soundState=='on')
...
this.speed+=ig.system.tick*(150/this.speed);this.screen.y-=ig.system.tick*this.speed;this.score+=ig.system.tick*this.speed;this.player.pos.y-=ig.system.tick*this.speed;var walls=ig.game.getEntitiesByType(EntityWall);var coins=ig.game.getEntitiesByType(EntityCoin);if(this.screen.y<-1){var row=this.getRow();this.screen.y+=8;this.player.pos.y+=8;for(var k=0;k<=8;k++){if(row[k]){var x,y;if(Math.random()>0.80){y=-28;x=k*8;this.spawnEntity(EntityCoin,x,y);}else{y=-28;x=k*8;this.spawnEntity(EntityWall,x,y);}}}
...
return;},draw:function(){this.backdrop.draw();this.font.draw('PIXEL RACE',ig.system.width/2,22,ig.Font.ALIGN.CENTER);this.font.draw('Press Enter',ig.system.width/2,38,ig.Font.ALIGN.CENTER);this.font.draw('to Start',ig.system.width/2,46,ig.Font.ALIGN.CENTER);this.font.draw('coins 150P',ig.system.width/2,66,ig.Font.ALIGN.CENTER);this.parent();}});ig.main('#canvas',PixelRaceStarter,30,64,96,5,PixelRaceLoader);});
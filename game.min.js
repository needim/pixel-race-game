...
ctx.putImageData(px,0,0);}});window.PixelRaceGame=ig.Game.extend({clearColor:null,gravity:0,player:null,isSubmitted:false,score:0,speed:30,backdrop:new FullsizeBackdrop('media/backdrop.png'),font:new ig.Font('media/04b03.font.png'),gameOverSound:new ig.Sound('media/gameover.ogg'),loopSound:new ig.Sound('media/loop.ogg'),init:function(){ig.input.bind(ig.KEY.LEFT_ARROW,'left');ig.input.bind(ig.KEY.RIGHT_ARROW,'right');ig.input.bind(ig.KEY.ENTER,'ok');ig.music.add('media/loop.ogg');ig.music.play();this.player=this.spawnEntity(EntityPlayer,ig.system.width/2-2,ig.system.height-10);},getRow:function(){var row=[];for(var x=0;x<8;x++){row[x]=Math.random()>0.98?1:0;}
...
this.parent();},draw:function(){this.backdrop.draw();if(this.gameOver){this.font.draw('PIXEL RACE',ig.system.width/2,22,ig.Font.ALIGN.CENTER);this.font.draw('Press Enter',ig.system.width/2,38,ig.Font.ALIGN.CENTER);this.font.draw('to Start',ig.system.width/2,46,ig.Font.ALIGN.CENTER);ig.music.stop();}
else{this.parent();}
this.font.draw(this.score.floor().toString(),ig.system.width-2,ig.system.height-6,ig.Font.ALIGN.RIGHT);this.font.draw(this.speed.floor().toString()+'km',2,ig.system.height-6,ig.Font.ALIGN.LEFT);}});window.PixelRaceStarter=ig.Game.extend({clearColor:null,gravity:0,player:null,isSubmitted:false,map:[],score:0,speed:30,backdrop:new FullsizeBackdrop('media/backdrop.png'),font:new ig.Font('media/04b03.font.png'),gameOverSound:new ig.Sound('media/gameover.ogg'),loopSound:new ig.Sound('media/loop.ogg'),init:function(){ig.input.bind(ig.KEY.LEFT_ARROW,'left');ig.input.bind(ig.KEY.RIGHT_ARROW,'right');ig.input.bind(ig.KEY.ENTER,'ok');ig.music.add('media/loop.ogg');},update:function(){if(ig.input.pressed('ok')){ig.system.setGame(PixelRaceGame);}
return;},draw:function(){this.backdrop.draw();this.font.draw('PIXEL RACE',ig.system.width/2,22,ig.Font.ALIGN.CENTER);this.font.draw('Press Enter',ig.system.width/2,38,ig.Font.ALIGN.CENTER);this.font.draw('to Start',ig.system.width/2,46,ig.Font.ALIGN.CENTER);this.parent();}});ig.main('#canvas',PixelRaceStarter,30,64,96,5,PixelRaceLoader);});